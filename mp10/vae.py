...
class VariationalAutoencoder(object):
    ...
    def _sample_z(self, z_mean, z_log_var):
        ...
        z = z_mean + epsilon * tf.sqrt(tf.exp(z_log_var))
    ...
    def _encoder(self, x):
        ...
        z_log_var = None
        ####### Implementation Here ######
        layer1 = fully_connected(x, num_outputs=100, activation_fn=tf.nn.softplus)
        layer2 = fully_connected(layer1, num_outputs=50, activation_fn=tf.nn.softplus)
        z_mean = fully_connected(layer2, num_outputs=self._nlatent, activation_fn=None)
        z_log_var = fully_connected(layer2, num_outputs=self._nlatent, activation_fn=None)
    ...
    def _decoder(self, z):
        ...
        f = None
        ####### Implementation Here ######
        layer1 = fully_connected(z, num_outputs=50, activation_fn=tf.nn.softplus)
        layer2 = fully_connected(layer1, num_outputs=100, activation_fn=tf.nn.softplus)
        f = fully_connected(layer2, num_outputs=self._ndims, activation_fn=None)
        f = tf.sigmoid(f)
        return f
    ...
    def _latent_loss(self, z_mean, z_log_var):
        ...
        latent_loss = None
        ####### Implementation Here ######
        result =  tf.constant(-1/2) -tf.log(tf.sqrt(tf.exp(z_log_var))) + (tf.exp(z_log_var) + z_mean**2)/2
        result1 = tf.reduce_sum(result, axis=1)
        latent_loss = tf.reduce_mean(result1, axis=0)
    ...
    def _reconstruction_loss(self, f, x_gt):
        ...
        result = (f-x_gt)**2
        result1 = tf.reduce_sum(result, axis=1)
        recon_loss = tf.reduce_mean(result1, axis=0)
    ...
    def loss(self, f, x_gt, z_mean, z_log_var):
        ...
        total_loss = None
        ####### Implementation Here ######
        latent = self._latent_loss(z_mean, z_log_var)
        recon = self._reconstruction_loss(f, x_gt)
        total_loss = latent + recon
