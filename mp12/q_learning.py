...
def get_action_index(readout_t, epsilon, t):
    ...
    number = len(readout_t)
    action_index = 0
    rand = np.random.randint(3)
    boundary = np.random.rand()
    # print('readout t', readout_t)

    if t <= OBSERVE:
        action_index =  rand
    elif boundary <= epsilon:
        action_index =  rand
    else:
...
def run_selected_action(a_t, s_t, game_state):
    ...
    """
    image, r_t, terminal = game_state.frame_step(a_t)
...
def compute_target_q(r_batch, readout_j1_batch, terminal_batch):
    ...
    for i in range(0, len(terminal_batch)):
        ...
        if terminal_batch[i] == True:
            ...
            result = r_batch[i] + GAMMA*np.max(readout_j1_batch[i])
            target_q_batch.append(result)
...
def trainNetwork(s, readout, sess):
    ...
    while True:
        ...
        if (t > OBSERVE):
            ...
            # print('readout_j1_batch', readout_j1_batch)
