...
class LinearMRF(object):
    ...
    def get_unary_features(self, img):
        ...
        length = img.shape[0]
        # print('length', length)
        first_row = np.zeros((1,length))
        second_row = np.ones((1,length))
        mask1 = img == first_row
        # print('mask1', mask1)
        mask2 = img == second_row

        result = np.vstack((mask1, mask2)).T
    ...
    def get_pairwise_features(self):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        zero_array = np.zeros((1,len(self.pairs)))
        ones_array = np.ones((1,len(self.pairs)))
        result = np.vstack((ones_array, zero_array))
        result = np.vstack((result, zero_array))
        result = np.vstack((result, ones_array)).T
        # print('result', result)
        return result
    ...
    def calculate_unary_potentials(self, unary_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        potentials = self.unary_weight * unary_features
        # print(potentials)
        return potentials
    ...
    def build_training_obj(self, img_features, unary_beliefs, pair_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        length = len(unary_beliefs)
        pairwise = self.get_pairwise_beliefs(img_features)
        new_img_features = tf.convert_to_tensor(img_features)
        new_img_features = tf.cast(new_img_features, unary_potentials[0].dtype)
        new_pairwise = tf.convert_to_tensor(pairwise)
        new_pairwise = tf.cast(new_pairwise, unary_potentials[0].dtype)

        for i in range(length):
            first_part = tf.reduce_sum(unary_beliefs[i]*unary_potentials[i]) + tf.reduce_sum(pair_beliefs[i]*pairwise_potentials)
            second_part = tf.reduce_sum(new_img_features*unary_potentials[i])
            obj = obj + first_part - second_part
        obj -= tf.reduce_sum(new_pairwise*pairwise_potentials)
        return obj
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        for i in range(length):
            ele = list[i]
            score0 = self.calculate_local_score(ele, 0, unary_beliefs, unary_pots, pairwise_pots)
            score1 = self.calculate_local_score(ele, 1, unary_beliefs, unary_pots, pairwise_pots)
            if score0 > score1:
                unary_beliefs[ele,:] = np.array([1,0])
            else:
                unary_beliefs[ele,:] = np.array([0,1])
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        #######################
        if assignment == 0:
            # unary_beliefs[node,:] = np.array([1,0])
            score += unary_potentials[node][0]
        elif assignment == 1:
            # unary_beliefs[node,:] = np.array([0,1])
            score += unary_potentials[node][1]
        ...
        for ele in neighbors:
            ...
            index = (node, ele)
            # print('index', index)
            pair_index = self.pair_inds[index]
            # print('pair index', pair_index)
            new_pair = self.pairs[pair_index]
            # print('new pair 0', new_pair)
            # if
            a = np.where(unary_beliefs[ele] == 1)[0][0]

            if assignment == 0 and a == 0:
                score += pairwise_potentials[pair_index][0]
            elif assignment == 0 and a == 1:
                score += pairwise_potentials[pair_index][1]
            elif assignment == 1 and a == 0:
                score += pairwise_potentials[pair_index][2]
            elif assignment == 1 and a == 1:
                score += pairwise_potentials[pair_index][3]
        return score
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        difference = new_unary_beliefs == old_unary_beliefs
        # print('diff', difference)
        ratio = 1 - np.sum(difference)/(difference.shape[0]*difference.shape[1])
        # print('ratio', ratio)
        if float(ratio) < convergence_margin:
            return True
        else:
            return False
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        for i in range(len(self.pairs)):
            ...
            word1 = ele[0]
            word2 = ele[1]
            if unary_beliefs[word1][0] == 1 and unary_beliefs[word2][0] == 1:
                result[i][0] = 1
            elif unary_beliefs[word1][0] == 1 and unary_beliefs[word2][1] == 1:
                result[i][1] = 1
            elif unary_beliefs[word1][1] == 1 and unary_beliefs[word2][0] == 1:
                result[i][2] = 1
            elif unary_beliefs[word1][1] == 1 and unary_beliefs[word2][1] == 1:
                result[i][3] = 1
        return result
